#!/usr/bin/env bash
#
# Interactive installer for go-display-driver.
#
...
main() {
  ...
  echo
  info "about to install go-display-driver as a systemd service (${SERVICE_NAME}) with:"
...
Environment=SPRITE_PATH=${SPRITE_PATH}
...
Environment=MESSAGE_BUS_QUEUE_NAME=${MESSAGE_BUS_QUEUE_NAME}
Environment=KEYBOARD_EXCHANGE=${KEYBOARD_EXCHANGE}
Environment=KEYBOARD_ROUTING_KEY=${KEYBOARD_ROUTING_KEY}
Environment=KEYBOARD_QUEUE_NAME=${KEYBOARD_QUEUE_NAME}
