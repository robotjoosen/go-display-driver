#!/usr/bin/env bash
#
# Updates an existing go-display-driver install to the latest GitHub release.
#
...
# first if go-display-driver isn't installed yet.
#
# Usage: run this directly on the target device:
#   ./scripts/update.sh
#
# Requires: curl, sudo, systemctl. Expects GitHub releases at
# github.com/robotjoosen/go-display-driver to publish binary assets named
# display-driver-linux-arm64 and display-driver-linux-arm.

set -euo pipefail

REPO="robotjoosen/go-display-driver"
SERVICE_NAME="display_driver"
INSTALL_PATH="/usr/local/bin/display-driver"
UNIT_PATH="/etc/systemd/system/${SERVICE_NAME}.service"
VERSION_MARKER="/usr/local/bin/.display-driver-version"

# Canonical set of env vars go-display-driver expects, with their defaults
# from cmd/app/setup.go -- used only for whatever's missing from the
# existing unit.
ENV_KEYS=(MODE LOG_LEVEL SPRITE_PATH MESSAGE_BUS_URL MESSAGE_BUS_EXCHANGE MESSAGE_BUS_ROUTING_KEY MESSAGE_BUS_QUEUE_NAME KEYBOARD_EXCHANGE KEYBOARD_ROUTING_KEY KEYBOARD_QUEUE_NAME)
declare -A ENV_DEFAULTS=(
  [MODE]="PROD"
  [LOG_LEVEL]="INFO"
  [SPRITE_PATH]="~/.config/go-display-driver/sprites"
  [MESSAGE_BUS_URL]="amqp://guest:guest@localhost:5672"
  [MESSAGE_BUS_EXCHANGE]="health"
  [MESSAGE_BUS_ROUTING_KEY]="health.ping"
  [MESSAGE_BUS_QUEUE_NAME]="display-driver"
  [KEYBOARD_EXCHANGE]="keyboard"
  [KEYBOARD_ROUTING_KEY]="keyboard.event"
  [KEYBOARD_QUEUE_NAME]="display-keyboard"
)
...
detect_arch() {
  case "$(uname -m)" in
    ...
    *)
      error "unsupported architecture: $(uname -m) (go-display-driver only publishes linux/arm64 and linux/arm binaries)"
      exit 1
      ;;
  esac
}
...
release_asset_url() {
  local release_json="$1" goarch="$2" asset_name="display-driver-linux-${goarch}"
  local url
...
main() {
  ...
  if [ ! -f "$INSTALL_PATH" ] || [ ! -f "$UNIT_PATH" ]; then
    error "go-display-driver doesn't look installed yet (missing ${INSTALL_PATH} or ${UNIT_PATH})"
  ...
  info "looking up the latest go-display-driver release..."
  ...
  echo
  info "about to update go-display-driver (${current_tag} -> ${tag}) with:"
  for key in "${ENV_KEYS[@]}"; do
    printf '    %-24s %s\n' "$key" "${final_env[$key]}"
  done
  echo
...
Environment=SPRITE_PATH=${final_env[SPRITE_PATH]}
...
Environment=MESSAGE_BUS_QUEUE_NAME=${final_env[MESSAGE_BUS_QUEUE_NAME]}
Environment=KEYBOARD_EXCHANGE=${final_env[KEYBOARD_EXCHANGE]}
Environment=KEYBOARD_ROUTING_KEY=${final_env[KEYBOARD_ROUTING_KEY]}
Environment=KEYBOARD_QUEUE_NAME=${final_env[KEYBOARD_QUEUE_NAME]}
