#!/usr/bin/env bash
#
# Uninstalls go-display-driver: stops and disables the systemd service, then
# removes the unit file, binary, and version marker left by install.sh /
# update.sh.
#
# Usage: run this directly on the device where go-display-driver is installed:
...
main() {
  ...
  if [ ! -f "$UNIT_PATH" ] && [ ! -f "$INSTALL_PATH" ]; then
    warn "go-display-driver doesn't look installed (no ${UNIT_PATH} or ${INSTALL_PATH}) -- nothing to do"
    exit 0
  fi

  echo
  info "about to uninstall go-display-driver:"
  ...
  echo
  info "done. go-display-driver has been uninstalled."
}
